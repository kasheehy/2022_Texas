...
# Species:Sailfin
exp(-1.51) # odds 0.22091
1/exp(-1.51) # There is a 4.53% decrease in the probability of observing parasites in Sailfins

# site:Weslaco
exp(-2.77) # odds 0.062662
1/exp(-2.77) # There is a 15.96% decrease in the likelihood of observing parasites in fish from Weslaco.
```
...
# test which distribution is best
mod_site_gaussian_br <- glm(totalpara ~ species,
                        family = gaussian(link = "identity"),
                        data = parasite_data_nozeros_br
                        )

mod_site_poisson_br <- glm(totalpara ~ species,
                        family = poisson(link = "log"),
                        data = parasite_data_nozeros_br
                        )

mod_site_nbin_br <- glm.nb(totalpara ~ species,
                        data = parasite_data_nozeros_br
                        )

# let's compare dispersion
check_overdispersion(mod_site_gaussian_br) # data is not over dispersed
check_overdispersion(mod_site_poisson_br) # data is over dispersed
check_overdispersion(mod_site_nbin_br) # data is not over dispersed

lrtest(mod_site_gaussian_br, mod_site_nbin_br) # there is a sig difference between the two, with nbin having a lower loglik
```

And now we evaluate model fit with the performance package
```{r}
model_performance(mod_site_nbin_br)
```
...
# test which distribution is best
mod_site_gaussian_wes <- glm(totalpara ~ species,
                        family = gaussian(link = "identity"),
                        data = parasite_data_nozeros_wes
                        )

mod_site_poisson_wes <- glm(totalpara ~ species,
                        family = poisson(link = "log"),
                        data = parasite_data_nozeros_wes
                        )

mod_site_nbin_wes <- glm.nb(totalpara ~ species,
                        data = parasite_data_nozeros_wes
                        )

# let's compare dispersion
check_overdispersion(mod_site_gaussian_wes) # data is not over dispersed
check_overdispersion(mod_site_poisson_wes) # data is over dispersed
check_overdispersion(mod_site_nbin_wes) # data is over dispersed

lrtest(mod_site_gaussian_wes, mod_site_nbin_wes) # there is a sig difference between the two, with nbin having a lower loglik

```

And now we evaluate model fit with the performance package
```{r}
model_performance(mod_site_nbin_wes)
```
...
# totalpara
exp(-0.002) # odds 0.998002
1/exp(-0.002) # For every increase of 1 parasite, there is a 1.00% decrease in the amount of time spent in the open.

## binomial model ##

# Species:Sailfin
exp(-1.404) # odds 0.2456125
1/exp(-1.404) # There is a 4.07% decrease in the likelihood that Sailfins spend any time in the open.


# site:Weslaco
exp(1.006) # odds 2.734641
1/exp(1.006) # There is a 0.36% increase in the likelihood that fish from Weslaco spend any time in the open.
```
...
```{r}
# FULL model, with three-way interaction
mod_beh_full_br <- lm(total.time.open.after ~ species * totalpara + standard.length,
                   data = br_data)

# to run all the tests in DHARMa, you first have to simulate your residuals
sim.mod_beh_br <- DHARMa::simulateResiduals(mod_beh_full_br)

# then you can plot them
plot(sim.mod_beh_br) # these both look not great

# gives you a bunch of tests of dispersion etc
DHARMa::testResiduals(sim.mod_beh_br) # dispersion looks ok, but the QQ plot is bad and outlier test is significant
...
```{r}
# filtering data to exclude zeros
beh_data_nozeros_br <- br_data %>% 
  filter(total.time.open.after > 0)

# test which distribution is best
mod_site_gaussian_br <- glm(total.time.open.after ~ species,
                        family = gaussian(link = "identity"),
                        data = beh_data_nozeros_br
                        )

mod_site_poisson_br <- glm(total.time.open.after ~ species,
                        family = poisson(link = "log"),
                        data = beh_data_nozeros_br
                        )

mod_site_nbin_br <- glm.nb(total.time.open.after ~ species,
                        data = beh_data_nozeros_br
                        )
...
# checking assumptions for the combined model
# First we have to simulate your residuals
sim.output <- DHARMa::simulateResiduals(mod_site_nbin_br)
...
check_overdispersion(mod_site_nbin_br) # no overdispersion detected. dispersion ratio = 0.747 p = 0.824
```
...
```{r}
# make column with binary value for infection status
br_data$binary.status <- ifelse(br_data$total.time.open.after == 0,0,1)

# now, let's test which distribution is best

mod_site_zi_binom_br <- glm(binary.status ~ species,
                         family = binomial(link = "logit"),
                        data = br_data
                        )

mod_site_zi_poisson_br <- glm(binary.status ~ species,
                        family = poisson(link = "log"),
                        data = br_data
                        )
                        

mod_site_zi_nbin_br <- glm.nb(binary.status ~ species,
                          data = br_data) # warning error about iteration limit indicates data is underdispersed. This is confirmed with the dispersion test below.

# let's compare dispersion
check_overdispersion(mod_site_zi_binom_br) # data is not over dispersed
check_overdispersion(mod_site_zi_poisson_br) # data is not over dispersed
check_overdispersion(mod_site_zi_nbin_br) # data is under dispersed

lrtest(mod_site_zi_binom_br, mod_site_zi_poisson_br) # significant difference, and the binomial model has a lower log lik
```
...
# checking assumptions for the combined model
# First we have to simulate your residuals
sim.output <- DHARMa::simulateResiduals(mod_site_zi_binom_br)
...
# check dispersion of combined
check_overdispersion(mod_site_zi_binom_br) # no overdispersion detected. dispersion ratio = 1.023 p = 0.808
```

##### Conclusions
Let's take a look at our final models.
```{r}
summary(mod_site_nbin_br) # species not significant (est = 0.3357, SE = 0.42, p = 0.422)

summary(mod_site_zi_binom_br) # species not significant (est = -1.18, SE = 0.64, p = 0.068
```
...
```{r}
# FULL model, with three-way interaction
mod_beh_full_wes <- lm(total.time.open.after ~ species * totalpara + standard.length,
                   data = wes_data)

# to run all the tests in DHARMa, you first have to simulate your residuals
sim.mod_beh_wes <- DHARMa::simulateResiduals(mod_beh_full_wes)

# then you can plot them
plot(sim.mod_beh_wes) # these both look not great

# gives you a bunch of tests of dispersion etc
DHARMa::testResiduals(sim.mod_beh_wes) # dispersion looks ok, but the QQ plot is bad and outlier test is significant
...
# test which distribution is best
mod_site_gaussian_wes <- glm(total.time.open.after ~ species,
                        family = gaussian(link = "identity"),
                        data = beh_data_nozeros_wes
                        )

mod_site_poisson_wes <- glm(total.time.open.after ~ species,
                        family = poisson(link = "log"),
                        data = beh_data_nozeros_wes
                        )

mod_site_nbin_wes <- glm.nb(total.time.open.after ~ species,
                        data = beh_data_nozeros_wes
                        )
...
check_overdispersion(mod_site_gaussian_wes) # data is not over dispersed
check_overdispersion(mod_site_poisson_wes) # data is over dispersed
check_overdispersion(mod_site_nbin_wes) # data is not over dispersed

lrtest(mod_site_gaussian_wes, mod_site_nbin_wes) # there is a sig difference between the two, with nbin having a slightly lower loglik
```
...
# checking assumptions for the combined model
# First we have to simulate your residuals
sim.output <- DHARMa::simulateResiduals(mod_site_nbin_wes)
...
check_overdispersion(mod_site_nbin_wes) # no overdispersion detected. dispersion ratio = 0.667 p = 0.296
```
...
```{r}
# make column with binary value for infection status
wes_data$binary.status <- ifelse(wes_data$total.time.open.after == 0,0,1)

# now, let's test which distribution is best

mod_site_zi_binom_wes <- glm(binary.status ~ species,
                         family = binomial(link = "logit"),
                        data = wes_data
                        )

mod_site_zi_poisson_wes <- glm(binary.status ~ species,
                        family = poisson(link = "log"),
                        data = wes_data
                        )
                        

mod_site_zi_nbin_wes <- glm.nb(binary.status ~ species,
...
# let's compare dispersion
check_overdispersion(mod_site_zi_binom_wes) # data is not over dispersed
check_overdispersion(mod_site_zi_poisson_wes) # data is not over dispersed
check_overdispersion(mod_site_zi_nbin_wes) # data is under dispersed

lrtest(mod_site_zi_binom_wes, mod_site_zi_poisson_wes) # significant difference, and the binomial model has a lower log lik
```
...
# checking assumptions for the combined model
# First we have to simulate your residuals
sim.output <- DHARMa::simulateResiduals(mod_site_zi_binom_wes)
...
# check dispersion of combined
check_overdispersion(mod_site_zi_binom_wes) # no overdispersion detected. dispersion ratio = 1.012 p = 0.936
```

##### Conclusions
Let's take a look at our final models.
```{r}
summary(mod_site_nbin_wes) # species significant (est = 0.3903, SE = 0.1979, p = 0.0486)

summary(mod_site_zi_binom_wes) # species significant (est = -1.2792, SE = 0., p = 0.068
```
